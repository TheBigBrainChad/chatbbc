...
- **Renamed to ChatBBC.** ChatBBC installs beside Chat On Steroids under its own app id, starts with an empty `chatbbc` user data folder and speaks bridge protocol 15, so a leftover Chat On Steroids companion is refused rather than silently ignored. Recreate the three connectors in ChatGPT as **ChatBBC Core**, **ChatBBC Desktop** and **ChatBBC Plugins**, then refresh ChatGPT’s connector snapshot. Do not run both apps at once: they contend for the same bridge ports.

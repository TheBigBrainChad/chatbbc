...
description: Use when executing an implementation plan in the current session as the implementer yourself — your human partner chose inline execution, or ChatBBC workers are unavailable
---

# Executing Plans

Execute the plan yourself, task by task, in this session: no implementer
worker per task, no reviewer per task. One fresh-context review of the
...
**ChatBBC tools:** `read`, `apply_patch`, `exec_command`/`write_stdin`/`find`,
and `update_plan`. Helper scripts in this skill and in
/subagent-driven-development are inert files; run them through `exec_command`
when Command permission is on. A final-review worker is a star-topology
ChatGPT conversation and cannot spawn workers of its own.
...
- ChatBBC workers are unavailable: multi-agent is Off, no browser is attached,
  or the family is at its worker cap and a sleeping worker cannot take the work.
...
digraph process {
    ...
    subgraph cluster_per_task {
        ...
        "task-done: run tests, ledger the result; mark the plan step complete" [shape=box];
    }
...
Track progress in a ledger file, not only in `update_plan`. The plan view is a
live projection; the ledger is the record.
...
`update_plan` step per task. If the plan names a Spec, read that too: the spec is the
...
- Run this skill's `scripts/task-start PLAN_FILE N` through `exec_command`. It prints the brief
...
- Mark the matching `update_plan` step in_progress.
...
Run this skill's `scripts/task-done PLAN_FILE N BASE -- <test command>` through `exec_command`
...
mark the matching `update_plan` step complete and take the next task.

## Final Review

Run `../subagent-driven-development/scripts/review-package PLAN_FILE MERGE_BASE HEAD`
through `exec_command` (MERGE_BASE = the commit the branch started from, e.g.
`git merge-base main HEAD`) and review from the file it prints.

**With workers available:** spawn a reviewer worker using
...
made. Let ChatBBC pick the model: omit `model` and `reasoning_effort`
unless the user explicitly requested an override. This is the one fresh
context the whole run buys. Do not skip it, and do not replace it with
your own read of the diff.

**Without workers:** read code-reviewer.md and perform that review
yourself against the package, as a separate pass after the last task's
ledger line. Write `Final review: self-review (no worker)` to the

...
export function createBrowserControl(chrome, transport, protectedTab = () => false) {
  ...
  async function screenshot(state,args,command) {
    ...
    let result, screencast = false;
    try {
      // Chrome can indefinitely throttle background capture after input or layout changes.
      // A one-pixel capture-scoped screencast wakes that compositor without selecting the tab.
      await send(state,'Page.startScreencast',{format:'jpeg',quality:1,maxWidth:1,maxHeight:1,everyNthFrame:1},undefined,command);
      screencast = true;
      result = await send(state,'Page.captureScreenshot',{format:'jpeg',quality:80,fromSurface:true,captureBeyondViewport:args.fullPage === true,clip:{...rect,scale}},undefined,command);
    } finally {
      if (screencast && tabs.get(state.tabId) === state) await cleanup(chrome.debugger.sendCommand({tabId:state.tabId},'Page.stopScreencast'));
    }

...
async function buildAndRun() {
  ...
  fs.writeFileSync(path.join(fixture, 'index.html'), `<!doctype html><html lang="en"><head>
    <meta charset="utf-8"><title>ChatBBC Pet CPU Verification</title></head>
    <body><div class="app"><aside class="sidebar">ChatBBC</aside>
  ...
  // Only the label is ours; every remaining argument belongs to Electron. Forwarding them
  // matters here because this session needs --ozone-platform=x11 and --disable-gpu to run at
  // all, and a crashed GPU process otherwise looks like a failed measurement.
  const child = spawn(require('electron'), [__filename, label, ...process.argv.slice(3)],

...
- A read-only relay attempt targeting a signed-in ChatGPT tab timed out. Neither signed-in widget interaction nor installed-widget presentation was verified; the tagged CI later verified installed GUI startup, not that provider path. Source, Chromium fixture, tests, build, unpacked payload and installed startup remain separate evidence levels.

## Publication

- The first tagged dispatch [35865277672](https://github.com/TheBigBrainChad/chatbbc/actions/runs/35865277672) stopped before assembly: Linux x64's full suite timed out at `test/content-script.test.ts:1989` while awaiting a second overflow recapture (`issued` remained 1). Linux arm64 and native-source jobs passed; no release was created. The focused test passed locally on the unchanged commit. The timing failure's root cause was not established or hidden.
- The unchanged `v2.1.21` tag was retried in [35866230568](https://github.com/TheBigBrainChad/chatbbc/actions/runs/35866230568). Preflight, native sources, Linux x64/arm64 verification, packaging, native runtime smoke, installed DEB GUI startup, AppImage startup, candidate assembly and Publish all passed. The tag resolves to `d0ed3e162a8661ff6915d42261cc2ec9299bd7bf`.
- [Release v2.1.21](https://github.com/TheBigBrainChad/chatbbc/releases/tag/v2.1.21) is public and non-draft, with both Linux architectures' AppImage/DEB, the extension ZIP, native-source archive and `SHA256SUMS.txt`. The downloaded checksum manifest's SHA-256 `0c7d58519fc7bb9e77869f8a101e0588c29c0030d755668fcba3eaecb776a03b` matches GitHub's asset digest; its six entries match their published asset digests. Provider widget submission remains unverified and unsupported in this correction.

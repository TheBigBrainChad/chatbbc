...
async function fixture(owner = 'A', protectedPage = true) {
  ...
  const chrome = {
    ...
    debugger:{attach:vi.fn(async()=>{}),detach:vi.fn(async()=>{}),
      getTargets:vi.fn(async()=>[{tabId:17,attached:true}]),sendCommand:vi.fn(async(_target:unknown,_method:string,_params?:unknown)=>({}))}
  };
...
describe('browser extension release custody',()=>{
  ...
  it('wakes a background renderer only for screenshot capture',async()=>{
    const {chrome,control,command}=await fixture('A',false);
    ...
    chrome.debugger.sendCommand.mockImplementation(async(_target:unknown,method:string)=>{
      if(method==='Page.getLayoutMetrics') return {
        cssVisualViewport:{pageX:0,pageY:0,clientWidth:800,clientHeight:600,scale:1},
        cssContentSize:{x:0,y:0,width:800,height:1200}
      };
      if(method==='Page.startScreencast'){screencast=true;return {};}
      if(method==='Page.captureScreenshot'){
        if(!screencast) throw new Error('background renderer is asleep');
        return {data:Buffer.from('fixture image').toString('base64')};
      }
      if(method==='Page.stopScreencast'){screencast=false;return {};}
      return {};
    });
    await expect(control.execute({...command('list'),tool:'browser_screenshot',args:{tabId:17,fullPage:false}}))
      .resolves.toMatchObject({value:{width:800,height:600},image:{mimeType:'image/jpeg'}});
    expect(screencast).toBe(false);
  });

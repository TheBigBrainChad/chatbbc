...
function page() {
  dom = new JSDOM('<section role="tabpanel" aria-labelledby="settings-trigger-Plugins"><h2>ChatBBC Core</h2><button id="schema">Schema kopieren</button><footer><button id="refresh">Aktualisieren</button></footer></section>', { runScripts: 'outside-only', url: 'https://chatgpt.com/#settings/Plugins/plugin_asdk_app_synthetic' });
  ...
  const props = { connector: { id: 'asdk_app_synthetic', name: 'ChatBBC Core', app_metadata: { version_id: 'asdk_app_v_synthetic' }, owners: ['never-copy'] },
...
it('reads localized installed declarations and the unique native refresh action from provider state', async () => {
  const { api } = page(); const view = await api.pluginRefreshView('ChatBBC Core', [tool]);
...
it('does not substitute expected declarations for changed provider descriptions', async () => {
  ...
  expect((await api.pluginRefreshView('ChatBBC Core', [tool])).tools[0].description).toBe('Changed declaration.');
  props.actions.push(props.actions[0]!);
  expect(await api.pluginRefreshView('ChatBBC Core')).toBeNull();
});
it('uses an enrolled App ID through renames, but refuses mismatched IDs and loading schemas', async () => {
  ...
  expect(await api.pluginRefreshView('ChatBBC Core')).toBeNull();
  expect(await api.pluginRefreshView('ChatBBC Core', [], 'asdk_app_synthetic')).not.toBeNull();
  expect(await api.pluginRefreshView('ChatBBC Core', [], 'asdk_app_other')).toBeNull();
...
it.each([118, 257])('accepts %s declarations for the Plugins connector including its optional code-mode tool', async (count) => {
  const { api, props } = page();
  props.connector.name = 'ChatBBC Plugins';
  ...
  const view = await api.pluginRefreshView('ChatBBC Plugins');
  expect(view?.tools).toHaveLength(count);
});
it.each([['ChatBBC Plugins', 258], ['ChatBBC Core', 17]] as const)('retains the %s observation count guard', async (connector, count) => {
  const { api, props } = page();
...
it('refuses ambiguous native actions and never copies unrelated connector properties', async () => {
  ...
  await api.pluginRefreshView('ChatBBC Core'); expect(JSON.stringify(messages)).not.toContain('never-copy');
...
it('discovers exact installed rows across languages and preserves ambiguity', () => {
  ...
  expect(api.pluginInstalledButtons('ChatBBC Core')).toBeNull();
  panel.insertAdjacentHTML('beforeend', '<button><span data-testid="plugin-icon-wrapper"></span><div>ChatBBC Core</div><span>Alle zulassen</span></button>');
  expect(api.pluginInstalledButtons('ChatBBC Core')).toHaveLength(1);
  expect(api.pluginInstalledButtons('ChatBBC Desktop')).toEqual([]);
  panel.insertAdjacentHTML('beforeend', panel.querySelector('button')!.outerHTML);
  expect(api.pluginInstalledButtons('ChatBBC Core')).toHaveLength(2);
});
...
it('observes an exact installed card without a refresh action through the real Fiber bridge', async () => {
  ...
  refresh.remove();
  expect(await api.pluginRefreshView('ChatBBC Core', [tool])).toMatchObject({ appId: 'asdk_app_synthetic', tools: [tool], refresh: null });
  delete (dom.window.document.getElementById('schema') as any).__reactFiber$fixture.return;
  expect(await api.pluginRefreshView('ChatBBC Core', [tool])).toBeNull();
});
